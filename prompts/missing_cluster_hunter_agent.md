...
For `missing_cluster_hunter_agent`, the essential artifact is `missing cluster report`. That artifact exists to prevent this failure mode: An entire relevant branch may remain undiscovered because no query or frontier targeted it. The first directly named stage for this agent is Stage 18: Missing-Cluster Hunt and Adversarial Audit. The output must contain enough detail for a later agent to trust it, challenge it, or route the workflow back to a specific stage.
...
9. Write `workspace/work/deep-paper-search/agent_artifacts/coverage_audit_adversarial/missing_cluster_hunter_agent.md` only when this invocation creates a reusable artifact beyond the ledger row; write `workspace/work/deep-paper-search/agent_artifacts/coverage_audit_adversarial/missing_cluster_hunter_agent.json` when records should be parsed by another stage.

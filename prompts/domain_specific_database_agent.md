...
For `domain_specific_database_agent`, the essential artifact is `domain database result batch`. That artifact exists to prevent this failure mode: General-purpose indexes may fail to cover the target discipline's primary archive. The first directly named stage for this agent is Stage 5: Parallel Scholarly Source Retrieval. The output must contain enough detail for a later agent to trust it, challenge it, or route the workflow back to a specific stage.
...
9. Write `workspace/work/deep-paper-search/agent_artifacts/data_source_retrieval/domain_specific_database_agent.md` only when this invocation creates a reusable artifact beyond the ledger row; write `workspace/work/deep-paper-search/agent_artifacts/data_source_retrieval/domain_specific_database_agent.json` when records should be parsed by another stage.

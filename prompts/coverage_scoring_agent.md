...
- Stage 32: Coverage Scoring and Audit. Input: subreports and missing clusters. Output: coverage score and audit report. Why it matters: Prevent inflated self-assessment..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `coverage_scoring_agent`, the essential artifact is `coverage score`. That artifact exists to prevent this failure mode: The workflow may lack a coherent stopping signal. The first directly named stage for this agent is Stage 32: Coverage Scoring and Audit. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/coverage_audit_adversarial/coverage_scoring_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/coverage_audit_adversarial/coverage_scoring_agent.json`.

...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `deduplication_agent`, the essential artifact is `deduplicated corpus`. That artifact exists to prevent this failure mode: The same paper may be counted multiple times or fragmented across sources. The first directly named stage for this agent is Stage 15: Version Linking and Deduplication. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/normalization_dedup_identity/deduplication_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/normalization_dedup_identity/deduplication_agent.json`.

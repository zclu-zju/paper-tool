...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `metadata_canonicalization_agent`, the essential artifact is `canonical record candidates`. That artifact exists to prevent this failure mode: Noisy multi-source metadata may prevent reliable merging, screening, and graph construction. The first directly named stage for this agent is Stage 13: Metadata Normalization. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/normalization_dedup_identity/metadata_canonicalization_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/normalization_dedup_identity/metadata_canonicalization_agent.json`.

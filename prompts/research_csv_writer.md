...
4. Satisfy the requested total paper count when possible.
5. Satisfy the requested open-source/code count when required.
6. Prefer target-year papers and verified-code papers according to user requirements.
7. If repository cloning was requested, merge clone status and local paths from `repository_clones.csv`.
8. If paper artifact retrieval was requested, merge PDF/TeX download paths, compile status, and compiled PDF paths from `paper_artifacts.csv`.
9. Write `final_papers.csv`.
10. Write `research_summary.md`.
...
```csv
abstract_source,method_type,dataset_or_benchmark,metric_or_evaluation,scope_match,selection_reason,clone_url,license,pdf_url,tex_source_url,compiler_used,notes
```

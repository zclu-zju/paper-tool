...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `todo_executor_router_agent`, the essential artifact is `TODO routing decision`. That artifact exists to prevent this failure mode: A TODO may be executed by the wrong agent, bypass an existing stage, or produce an unusable artifact. The first directly named stage for this agent is Stage 35: TODO Execution Loop. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/todo_execution_control/todo_executor_router_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/todo_execution_control/todo_executor_router_agent.json`.

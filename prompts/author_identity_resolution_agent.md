...
- Stage 14: Identity Resolution. Input: canonical candidates. Output: author map and venue map. Why it matters: Prevent author and venue expansion errors..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `author_identity_resolution_agent`, the essential artifact is `author identity map`. That artifact exists to prevent this failure mode: Author expansion may include unrelated papers by a different person with the same name. The first directly named stage for this agent is Stage 14: Identity Resolution. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/normalization_dedup_identity/author_identity_resolution_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/normalization_dedup_identity/author_identity_resolution_agent.json`.

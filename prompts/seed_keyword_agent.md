...
**Primary artifact**: `workspace/work/deep-paper-search/terminology_keywords_queries/seed_keyword_agent.md`
**Optional structured artifact**: `workspace/work/deep-paper-search/terminology_keywords_queries/seed_keyword_agent.json`
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `seed_keyword_agent`, the essential artifact is `seed term set`. That artifact exists to prevent this failure mode: The workflow may lack an initial retrieval path into the literature. The first directly named stage for this agent is Stage 6: Initial Term Generation. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/terminology_keywords_queries/seed_keyword_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/terminology_keywords_queries/seed_keyword_agent.json`.

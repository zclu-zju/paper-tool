...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `experimental_setting_agent`, the essential artifact is `experimental setting table`. That artifact exists to prevent this failure mode: Papers may look relevant while using incompatible experimental conditions. The first directly named stage for this agent is Stage 20: Taxonomy Modeling. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/screening_taxonomy_evidence/experimental_setting_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/screening_taxonomy_evidence/experimental_setting_agent.json`.

...
7. Search literature only after requirements are READY and topic_scope.md is USER_CONFIRMED.
8. Preserve abstracts, citation counts, search queries, source URLs, artifact URLs, relevance rationales, and evidence use categories for every related-paper candidate.
...
10. Build evidence_map.csv linking manuscript claims and sections to literature evidence before any evidence-based review agent runs.
11. Run the reviewer panel and specialist diagnostic panel. Every major issue must cite manuscript location plus evidence_map rows or related papers.
12. Synthesize scores and decision from reviewer reports and specialist audits. Do not invent issues in synthesis.
13. If revision is requested, create a traceable revision plan, run one-to-one reviewer/reviser pairs over copied TeX files, update revision_ledger.xlsx with openpyxl tooling, and verify the revision.
14. Run integrity review.
15. If any stage is rejected, loop back to the specified target stage and retry automatically unless user input is required.

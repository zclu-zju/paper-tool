...
def process_rows(rows: list[dict[str, str]], selected: set[str], config: RequirementConfig, out_root: Path) -> list[dict[str, str]]:
    outputs: list[dict[str, str]] = []
    papers_root = out_root / "literature" / "papers"
    for row in rows:
        ...
        pdf_path = papers_root / "pdf" / pid / "paper.pdf"
        tex_dir = papers_root / "tex" / pid
        tex_path = tex_dir / "source"
        text_path = papers_root / "text" / pid / "extracted.txt"
        metadata_path = papers_root / "metadata" / pid / "artifact.json"
...
def write_outputs(rows: list[dict[str, str]], out_root: Path, config: RequirementConfig) -> None:
    reports = out_root / "reports"
    reports.mkdir(parents=True, exist_ok=True)
    csv_path = reports / "04_paper_artifacts.csv"
    ...
    md_path = reports / "04_paper_artifacts.md"
    lines = [
        "## STATUS",
        f"STATUS: {status}",
        "",
    ...
    lines.extend(
        [
            ...
            "- Convention mining input root: workspace/draft_paper_review/literature/papers/",
            "",
            ...
            f"- Literature Paper Root: {out_root / 'literature' / 'papers'}",
        ]
    )
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    parser.add_argument("--requirements", type=Path, default=Path("workspace/draft_paper_review/reports/00_requirements.md"))
    parser.add_argument("--candidates", type=Path, default=Path("workspace/draft_paper_review/reports/03_literature_candidates.csv"))
    parser.add_argument("--out-root", type=Path, default=Path("workspace/draft_paper_review"))
    args = parser.parse_args()

    config = read_requirements(args.requirements)
    rows = read_candidates(args.candidates)
    selected = select_rows(rows, config)
    outputs = process_rows(rows, selected, config, args.out_root)
    write_outputs(outputs, args.out_root, config)
    print(f"Wrote artifact report: {args.out_root / 'reports' / '04_paper_artifacts.csv'}")

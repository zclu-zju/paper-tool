...
12. Revision changes: if revision allowed, revised files are copies and changes trace to tasks.
13. Revision verification: required tasks verified or loopback specified.
14. Final threshold: score meets threshold or output is explicitly marked not ready.
15. Iteration policy: repeated stage/problem caps are respected and deferred issues are recorded instead of retried indefinitely.
16. Missing score policy: objective missing evidence is marked N/A/deferred according to requirements and is not silently converted into a numeric score.
17. Hard gates: no unresolved DA-CRITICAL, methodology <= 2, literature integration <= 2, originality <= 2 without repositioning, writing <= 2 without revision, unless explicitly deferred by policy and surfaced as not fully ready.
18. Loopback readiness: if any check fails, target exactly one stage.
...
Write `workspace/evidence_paper_review/reports/integrity_report.md`:

```markdown
# Evidence Paper Review Integrity Report
...
13. Revision Change Safety: [Yes/No/Not Required] - [Reason]
14. Revision Verification: [Yes/No/Not Required] - [Reason]
15. Final Threshold Status: [Yes/No] - [Reason]
16. Iteration Policy Compliance: [Yes/No] - [Reason]
17. Missing Score Policy Compliance: [Yes/No] - [Reason]
18. Loopback Readiness: [Yes/No] - [Reason]

...
If it contains `STATUS: NEEDS_USER_INPUT`, ask the listed questions and stop.
...
workspace/evidence_paper_review/reports/paper_artifacts.md
...
```text
workspace/evidence_paper_review/reports/specialist_audits/novelty_claim_audit.md
workspace/evidence_paper_review/reports/specialist_audits/terminology_consistency_audit.md
workspace/evidence_paper_review/reports/specialist_audits/field_style_audit.md
workspace/evidence_paper_review/reports/specialist_audits/professionalism_domain_precision_audit.md
workspace/evidence_paper_review/reports/specialist_audits/literature_positioning_audit.md
workspace/evidence_paper_review/reports/specialist_audits/argument_coherence_audit.md
workspace/evidence_paper_review/reports/specialist_audits/citation_reference_audit.md
workspace/evidence_paper_review/reports/specialist_audits/writing_quality_audit.md
workspace/evidence_paper_review/reports/specialist_audits/specialist_summary.md
```
...
## Stage 12: Manuscript Polishing And Revision

Run `manuscript-polisher-reviser` only when revision is allowed.

Outputs:

```text
workspace/evidence_paper_review/revision/
workspace/evidence_paper_review/reports/revision_changes.md
```

Revised files must be copies, not overwrites. Substantive edits must trace to the revision plan and evidence map.
...
The verifier checks each required revision item, evidence support, citation consistency, and whether new unsupported claims were introduced.
...
2. Compute an `issue_signature` from:
   ...
   - evidence gap;
   - requested action.
3. Read `workspace/evidence_paper_review/reports/iteration_log.md` and count previous retries with the same `issue_signature`.
...
2. Classify the unresolved issue:
   - `OBJECTIVE_LIMITATION`: cannot be fixed from the current manuscript/workspace, such as missing experiment results, missing raw data, unavailable proprietary dataset, absent IRB approval, missing model checkpoint, or no TeX source when only a PDF was provided.
...
3. Apply requirements policy:
   - `DEFER_AND_CONTINUE`: write the issue to `workspace/evidence_paper_review/reports/deferred_issues.md`, exclude it from further loopback, and continue with downstream stages.
...
Write or update `workspace/evidence_paper_review/reports/deferred_issues.md`:
...
- Keep all generated outputs under `workspace/evidence_paper_review/`.

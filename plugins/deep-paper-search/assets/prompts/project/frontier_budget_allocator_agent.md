...
- Stage 21: Seed Paper Selection. Input: taxonomies and quality signals. Output: seed papers and frontier budgets. Why it matters: Choose representative seeds for multi-path expansion..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `frontier_budget_allocator_agent`, the essential artifact is `frontier budget plan`. That artifact exists to prevent this failure mode: One attractive frontier may consume the budget while more valuable discovery paths remain unexplored. The first directly named stage for this agent is Stage 21: Seed Paper Selection. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/orchestration_state_reproducibility/frontier_budget_allocator_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/orchestration_state_reproducibility/frontier_budget_allocator_agent.json`.

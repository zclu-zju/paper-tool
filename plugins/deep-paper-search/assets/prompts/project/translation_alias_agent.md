...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `translation_alias_agent`, the essential artifact is `alias map`. That artifact exists to prevent this failure mode: Literal translation from the user language may miss the vocabulary used by the target community. The first directly named stage for this agent is Stage 6: Initial Term Generation. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/terminology_keywords_queries/translation_alias_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/terminology_keywords_queries/translation_alias_agent.json`.

...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `failure_triage_agent`, the essential artifact is `recovery action`. That artifact exists to prevent this failure mode: Tool or access failures may be mistaken for evidence that no relevant literature exists. This agent is invoked as a support role when its phase artifact is required by another stage or audit. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/orchestration_state_reproducibility/failure_triage_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/orchestration_state_reproducibility/failure_triage_agent.json`.

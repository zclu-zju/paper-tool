...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `figure_table_signal_agent`, the essential artifact is `figure and table signal records`. That artifact exists to prevent this failure mode: Important experimental terminology may be missed because it appears only in figures or tables. The first directly named stage for this agent is Stage 12: Full Text Parsing. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/full_text_content/figure_table_signal_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/full_text_content/figure_table_signal_agent.json`.

...
For `todo_planner_agent`, the essential artifact is `initial TODO queue`. That artifact exists to prevent this failure mode: TODO mode may start without concrete executable tasks or with tasks that do not map to the workflow. The first directly named stage for this agent is Stage 23: Optional TODO Execution Loop. The output must contain enough detail for a later agent to trust it, challenge it, or route the workflow back to a specific stage.
...
9. Write `workspace/work/deep-paper-search/agent_artifacts/todo_execution_control/todo_planner_agent.md` only when this invocation creates a reusable artifact beyond the ledger row; write `workspace/work/deep-paper-search/agent_artifacts/todo_execution_control/todo_planner_agent.json` when records should be parsed by another stage.

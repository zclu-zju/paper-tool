...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `keyword_gap_auditor_agent`, the essential artifact is `keyword gap report`. That artifact exists to prevent this failure mode: Keyword coverage may appear broad while important dimensions remain absent. The first directly named stage for this agent is Stage 8: Query Probing. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/terminology_keywords_queries/keyword_gap_auditor_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/terminology_keywords_queries/keyword_gap_auditor_agent.json`.

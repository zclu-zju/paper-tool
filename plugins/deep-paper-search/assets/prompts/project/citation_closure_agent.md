...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `citation_closure_agent`, the essential artifact is `citation closure report`. That artifact exists to prevent this failure mode: Citation expansion may stop while important connected papers remain unvisited. The first directly named stage for this agent is Stage 30: Coverage Analysis. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/coverage_audit_adversarial/citation_closure_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/coverage_audit_adversarial/citation_closure_agent.json`.

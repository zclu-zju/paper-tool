...
- Stage 29: Evidence Graph Construction. Input: validated corpus, terms, taxonomies, and frontiers. Output: evidence graph. Why it matters: Provide structure for coverage audit..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `evidence_graph_agent`, the essential artifact is `evidence graph`. That artifact exists to prevent this failure mode: Coverage gaps, provenance paths, and cluster relationships may be impossible to evaluate. The first directly named stage for this agent is Stage 29: Evidence Graph Construction. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/screening_taxonomy_evidence/evidence_graph_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/screening_taxonomy_evidence/evidence_graph_agent.json`.

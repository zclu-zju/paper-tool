...
- Stage 26: Standards and Engineering Vocabulary Expansion. Input: core terms and domain. Output: standards frontier. Why it matters: Add engineering aliases and application vocabulary..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `standard_patent_agent`, the essential artifact is `standards and patents frontier`. That artifact exists to prevent this failure mode: Engineering terminology may remain disconnected from scholarly terminology. The first directly named stage for this agent is Stage 26: Standards and Engineering Vocabulary Expansion. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/expansion_frontiers/standard_patent_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/expansion_frontiers/standard_patent_agent.json`.

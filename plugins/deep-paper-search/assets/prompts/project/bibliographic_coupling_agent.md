...
- Stage 22: Citation Expansion. Input: seed papers. Output: citation frontiers. Why it matters: Find papers that keyword search misses..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `bibliographic_coupling_agent`, the essential artifact is `bibliographic coupling frontier`. That artifact exists to prevent this failure mode: Parallel work with shared foundations but no direct citation edge may be missed. The first directly named stage for this agent is Stage 22: Citation Expansion. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/expansion_frontiers/bibliographic_coupling_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/expansion_frontiers/bibliographic_coupling_agent.json`.

...
For `venue_track_agent`, the essential artifact is `venue frontier`. That artifact exists to prevent this failure mode: Community papers using different wording may be missed by term-based search. The first directly named stage for this agent is Stage 13: Venue and Community Expansion. The output must contain enough detail for a later agent to trust it, challenge it, or route the workflow back to a specific stage.
...
9. Write `workspace/work/deep-paper-search/agent_artifacts/expansion_frontiers/venue_track_agent.md` only when this invocation creates a reusable artifact beyond the ledger row; write `workspace/work/deep-paper-search/agent_artifacts/expansion_frontiers/venue_track_agent.json` when records should be parsed by another stage.

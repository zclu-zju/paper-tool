...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `stop_condition_validator_agent`, the essential artifact is `stop validation`. That artifact exists to prevent this failure mode: The system may publish final results before required quality gates are satisfied. The first directly named stage for this agent is Stage 33: Adversarial Challenge and Iteration Decision. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/coverage_audit_adversarial/stop_condition_validator_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/coverage_audit_adversarial/stop_condition_validator_agent.json`.

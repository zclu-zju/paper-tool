...
For `scholarly_web_fallback_agent`, the essential artifact is `scholarly web result batch`. That artifact exists to prevent this failure mode: Relevant work not well indexed by APIs may remain invisible. The first directly named stage for this agent is Stage 5: Parallel Scholarly Source Retrieval. The output must contain enough detail for a later agent to trust it, challenge it, or route the workflow back to a specific stage.
...
9. Write `workspace/work/deep-paper-search/agent_artifacts/data_source_retrieval/scholarly_web_fallback_agent.md` only when this invocation creates a reusable artifact beyond the ledger row; write `workspace/work/deep-paper-search/agent_artifacts/data_source_retrieval/scholarly_web_fallback_agent.json` when records should be parsed by another stage.

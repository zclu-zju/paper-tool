...
**Detailed artifact path**: `workspace/work/deep-paper-search/agent_artifacts/intent_definition_boundaries/intent_decomposition_agent.md`
**Optional structured artifact path**: `workspace/work/deep-paper-search/agent_artifacts/intent_definition_boundaries/intent_decomposition_agent.json`
...
For `intent_decomposition_agent`, the essential artifact is `intent frame`. That artifact exists to prevent this failure mode: The initial request may be treated as a single keyword instead of a structured research intent. The first directly named stage for this agent is Stage 2: Domain, Scope, and Depth Contract. The output must contain enough detail for a later agent to trust it, challenge it, or route the workflow back to a specific stage.
...
9. Write `workspace/work/deep-paper-search/agent_artifacts/intent_definition_boundaries/intent_decomposition_agent.md` only when this invocation creates a reusable artifact beyond the ledger row; write `workspace/work/deep-paper-search/agent_artifacts/intent_definition_boundaries/intent_decomposition_agent.json` when records should be parsed by another stage.

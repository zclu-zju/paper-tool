...
For `near_miss_mining_agent`, the essential artifact is `near-miss signals`. That artifact exists to prevent this failure mode: Boundary papers may be discarded even though they reveal hidden search paths. The first directly named stage for this agent is Stage 8: Relevance Screening and Near-Miss Mining. The output must contain enough detail for a later agent to trust it, challenge it, or route the workflow back to a specific stage.
...
9. Write `workspace/work/deep-paper-search/agent_artifacts/screening_taxonomy_evidence/near_miss_mining_agent.md` only when this invocation creates a reusable artifact beyond the ledger row; write `workspace/work/deep-paper-search/agent_artifacts/screening_taxonomy_evidence/near_miss_mining_agent.json` when records should be parsed by another stage.

...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `forward_citation_agent`, the essential artifact is `forward citation frontier`. That artifact exists to prevent this failure mode: The corpus may omit later improvements, replications, and recent branches. The first directly named stage for this agent is Stage 22: Citation Expansion. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/expansion_frontiers/forward_citation_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/expansion_frontiers/forward_citation_agent.json`.

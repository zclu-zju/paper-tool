...
- Stage 35: TODO Execution Loop. Input: config, calibration result, current state, and active TODO queue. Output: updated TODO queue, done TODO log, and continuation decision. Why it matters: Continue TODO-mode execution until no useful executable task remains..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `todo_completion_verifier_agent`, the essential artifact is `TODO completion verdict`. That artifact exists to prevent this failure mode: A TODO may be removed from the active queue even though no valid completion evidence exists. The first directly named stage for this agent is Stage 35: TODO Execution Loop. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/todo_execution_control/todo_completion_verifier_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/todo_execution_control/todo_completion_verifier_agent.json`.

...
- Stage 8: Query Probing. Input: draft queries. Output: probe report and keyword gap report. Why it matters: Detect bad queries before large-scale retrieval..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `query_probe_agent`, the essential artifact is `query probe report`. That artifact exists to prevent this failure mode: Large retrieval budgets may be spent on noisy or misdirected queries. The first directly named stage for this agent is Stage 8: Query Probing. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/terminology_keywords_queries/query_probe_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/terminology_keywords_queries/query_probe_agent.json`.

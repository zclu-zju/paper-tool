...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `todo_continuation_auditor_agent`, the essential artifact is `TODO continuation audit`. That artifact exists to prevent this failure mode: The workflow may stop with unresolved executable work or continue indefinitely with low-value tasks. The first directly named stage for this agent is Stage 35: TODO Execution Loop. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/todo_execution_control/todo_continuation_auditor_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/todo_execution_control/todo_continuation_auditor_agent.json`.

...
- Stage 6: Initial Term Generation. Input: scope contract. Output: seed terms and alias map. Why it matters: Create the first retrieval entry points..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `controlled_vocabulary_agent`, the essential artifact is `controlled term candidates`. That artifact exists to prevent this failure mode: The workflow may ignore terms used by scholarly databases to index the target topic. The first directly named stage for this agent is Stage 6: Initial Term Generation. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/terminology_keywords_queries/controlled_vocabulary_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/terminology_keywords_queries/controlled_vocabulary_agent.json`.

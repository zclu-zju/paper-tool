...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `todo_selector_agent`, the essential artifact is `selected TODO`. That artifact exists to prevent this failure mode: The workflow may execute low-value tasks while important ready tasks remain untouched. The first directly named stage for this agent is Stage 35: TODO Execution Loop. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/todo_execution_control/todo_selector_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/todo_execution_control/todo_selector_agent.json`.

...
For `lab_institution_agent`, the essential artifact is `lab and institution frontier`. That artifact exists to prevent this failure mode: Series work not fully indexed by scholarly databases may remain undiscovered. The first directly named stage for this agent is Stage 12: Author and Lab Expansion. The output must contain enough detail for a later agent to trust it, challenge it, or route the workflow back to a specific stage.
...
9. Write `workspace/work/deep-paper-search/agent_artifacts/expansion_frontiers/lab_institution_agent.md` only when this invocation creates a reusable artifact beyond the ledger row; write `workspace/work/deep-paper-search/agent_artifacts/expansion_frontiers/lab_institution_agent.json` when records should be parsed by another stage.

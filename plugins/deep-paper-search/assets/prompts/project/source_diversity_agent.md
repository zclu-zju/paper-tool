...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `source_diversity_agent`, the essential artifact is `source diversity report`. That artifact exists to prevent this failure mode: The corpus may overrepresent one source, team, venue, or time period. The first directly named stage for this agent is Stage 30: Coverage Analysis. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/coverage_audit_adversarial/source_diversity_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/coverage_audit_adversarial/source_diversity_agent.json`.

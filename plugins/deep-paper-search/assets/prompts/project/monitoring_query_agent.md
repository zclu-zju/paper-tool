...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `monitoring_query_agent`, the essential artifact is `monitoring configuration`. That artifact exists to prevent this failure mode: The literature review may become stale without a targeted update path. The first directly named stage for this agent is Stage 34: Output and Monitoring. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/output_monitoring/monitoring_query_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/output_monitoring/monitoring_query_agent.json`.

...
- Stage 20: Taxonomy Modeling. Input: in-scope records. Output: taxonomies and setting table. Why it matters: Measure coverage by dimensions, not by count..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `dataset_metric_extraction_agent`, the essential artifact is `dataset and metric table`. That artifact exists to prevent this failure mode: The experimental context may be missing from taxonomy and coverage decisions. The first directly named stage for this agent is Stage 20: Taxonomy Modeling. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/screening_taxonomy_evidence/dataset_metric_extraction_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/screening_taxonomy_evidence/dataset_metric_extraction_agent.json`.

...
- Stage 15: Version Linking and Deduplication. Input: canonical candidates. Output: deduplicated corpus and version graph. Why it matters: Merge duplicates while preserving version relationships..
...
This prompt is long because the agent boundary must be operationally complete, not because filler text is acceptable. For `version_linking_agent`, the essential artifact is `version graph`. That artifact exists to prevent this failure mode: The workflow may duplicate the same paper or discard meaningful extended versions. The first directly named stage for this agent is Stage 15: Version Linking and Deduplication. The output must therefore contain enough detail for a later agent to decide whether to trust it, challenge it, or send the workflow back to a specific stage.
...
9. Write the artifact to `workspace/work/deep-paper-search/normalization_dedup_identity/version_linking_agent.md`. If your output contains records that would be easier to parse as data, also write `workspace/work/deep-paper-search/normalization_dedup_identity/version_linking_agent.json`.

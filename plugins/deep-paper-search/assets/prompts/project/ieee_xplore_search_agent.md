...
For `ieee_xplore_search_agent`, the essential artifact is `IEEE Xplore result batch`. That artifact exists to prevent this failure mode: Engineering-heavy topics may miss papers published in the dominant IEEE venues. The first directly named stage for this agent is Stage 5: Parallel Scholarly Source Retrieval. The output must contain enough detail for a later agent to trust it, challenge it, or route the workflow back to a specific stage.
...
9. Write `workspace/work/deep-paper-search/agent_artifacts/data_source_retrieval/ieee_xplore_search_agent.md` only when this invocation creates a reusable artifact beyond the ledger row; write `workspace/work/deep-paper-search/agent_artifacts/data_source_retrieval/ieee_xplore_search_agent.json` when records should be parsed by another stage.
